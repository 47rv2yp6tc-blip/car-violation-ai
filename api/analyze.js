...
// Gemini response schemas do not accept JSON Schema union types such as type: ['number','null'].
// Keep these fields as numbers in the model response and normalize missing/unknown values to null below.
const ANALYSIS_SCHEMA = {
  ...
  properties: {
    ...
    violations: { type: 'array', items: { type: 'object', properties: {
      violation_type: { type: 'string' }, confidence: { type: 'number' }, evidence: { type: 'string' }, reasoning: { type: 'string' }, missing_evidence: { type: 'array', items: { type: 'string' } }, fine_range: { type: 'string' }, fine_min: { type: 'number' }, fine_max: { type: 'number' }, regulation: { type: 'string' }, needs_human_review: { type: 'boolean' }
...
const promptFor = (count) => `你是具備影像理解能力的道路交通證據分析助手。這是同一事件的 ${count} 張照片，請綜合所有角度。先列出實際看得到的事實：車輛位置、車道、道路標線與停止線、紅綠燈狀態、交通標誌、車輛方向、停車位置、道路環境與照片品質；再只根據這些事實判斷可能違規。模糊、遮擋、過暗、主體太小、證據矛盾或缺少關鍵證據時，降低 confidence 或使用「無法從此照片確認」，並列出 missing_evidence，不要猜測。confidence 是影像證據信心，不是收到罰單的機率，也不是法律確定性，不得調高。若罰款無法可靠確認，fine_min 與 fine_max 請輸出 0，fine_range 輸出「需查證」；系統會將 0 視為未知。不要輸出車牌、臉部或其他個資。只回傳符合 schema 的 JSON，不要 Markdown。`;
...
export default async function handler(req, res) {
  ...
  try {
    const body = req.body || {}; const list = Array.isArray(body.images) && body.images.length ? body.images : [body.image]; const types = Array.isArray(body.mimeTypes) && body.mimeTypes.length ? body.mimeTypes : [body.mimeType];
    ...
    const parts = [];
    for (let i = 0; i < list.length; i += 1) { if (String(list[i]).length > MAX_DATA_URL_LENGTH) return jsonResponse(res, 413, { error: '照片過大，請壓縮後再試。' }); const image = parseImage(list[i], types[i]); if (!image) return jsonResponse(res, 400, { error: '只支援格式正確的 JPG、PNG 或 WEBP 圖片。' }); parts.push({ inlineData: image }); }
    ...
    let response;
    for (let attempt = 0; attempt < 2; attempt += 1) { const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), 55000); try { response = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(requestBody), signal: controller.signal }); } finally { clearTimeout(timeout); } if (![429, 500, 502, 503, 504].includes(response.status) || attempt === 1) break; await new Promise(resolve => setTimeout(resolve, 700)); }
    ...
    const text = raw?.candidates?.[0]?.content?.parts?.map(part => part.text || '').join('') || '{}'; const parsed = JSON.parse(text.replace(/^```json\s*|\s*```$/g, '').trim());
    ...
    const priced = violations.filter(item => item.fine_min !== null && item.fine_max !== null && !item.type.includes('無法')); const totalFineMin = priced.length ? priced.reduce((sum, item) => sum + item.fine_min, 0) : null; const totalFineMax = priced.length ? priced.reduce((sum, item) => sum + item.fine_max, 0) : null;
