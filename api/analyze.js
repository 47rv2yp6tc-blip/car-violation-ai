...
// The Gemini REST endpoint used by this project rejects the responseSchema
// shape for some model/API combinations. JSON mode plus strict prompt output
// is used instead, then the server validates and normalizes every field.
...
只回傳一個 JSON 物件，不要 Markdown、不要 ```，格式必須完全符合以下範例：
{"quality":{"usable":true,"issues":[],"explanation":""},"observed_facts":[],"violations":[{"violation_type":"","confidence":0,"evidence":"","reasoning":"","missing_evidence":[],"fine_range":"需查證","fine_min":0,"fine_max":0,"regulation":"","needs_human_review":true}],"summary":""}

欄位規則：confidence 是你對影像證據的原始 0-100 信心，不是收到罰單的機率，也不是法律確定性，不得調高。fine_min 與 fine_max 必須是數字；無法可靠確認罰款時使用 0，fine_range 使用「需查證」，系統會把 0 視為未知。不要輸出車牌、臉部或其他個資。`;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const requestBody = { contents: [{ parts: [{ text: promptFor(list.length) }, ...parts] }], generationConfig: { temperature: 0.05, responseMimeType: 'application/json' } };
    let response;
