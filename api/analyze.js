...
只回傳一個 JSON 物件，不要 Markdown 或程式碼圍欄，格式必須包含 quality、observed_facts、violations、summary 四個欄位。每個 violation 必須包含 violation_type、confidence、evidence、reasoning、missing_evidence、fine_range、fine_min、fine_max、regulation、needs_human_review。
...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      try {
        response = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(request(attempt === 0)), signal: controller.signal });
